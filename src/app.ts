...
<html>
  ...
  const loadInbox = async (sessionId: number, mailbox: string, mode: LoadMode) => {
    ...
    try {
      ...
      if (
        !isCurrentMailboxSession(sessionId, mailbox) ||
        requestId !== runtime.inboxRequestId
      ) {
        return;
      }
      ...
      setStatus(
        state.emails.length > 0
          ? `Inbox ready for ${mailbox}.`
          : `Waiting for emails in ${mailbox}...`
      );
    } catch (error) {
  ...
  const openEmail = async (summary: EmailSummary) => {
    ...
    try {
      ...
      if (
        !isCurrentMailboxSession(sessionId, mailbox) ||
        requestId !== runtime.emailRequestId ||
        state.selectedEmailId !== summary.id
      ) {
        return;
      }
  ...
  const InboxItem = component((props: { email: EmailSummary }) => html`
    <div
      class="${() => {
